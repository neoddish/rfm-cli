...
function getFromTree(node: any, arr: { path: string; file: File }[], dir: string) {
...
export class Structure {
  ...
  addFile(dir: string, file: File) {
    const { filename } = file;
    const dotPath = path.join('root', dir, filename).replace(/\./g, DOT_SUB).replace(/\//g, '.');
  ...
  outputList() {
    ...
    const formattedList = list.map(({ path, file }) => {
      ...
      return { path: path.replace(re, '.'), file };
    });
